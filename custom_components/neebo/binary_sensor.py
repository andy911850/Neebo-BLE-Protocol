...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async_add_entities([
        NeeboPlacementSensor(hub, entry.entry_id),
    ])

class NeeboPlacementSensor(BinarySensorEntity):
    def __init__(self, hub, entry_id):
        self._hub = hub
        self._attr_name = "Neebo Placement"
        ...
        # We can use the 'presence' or 'occupancy' device class, 
        # or no device class and just icon.
        self._attr_icon = "mdi:hand-back-right"

    @property
    def is_on(self):
        return self._hub.data.get("on_wrist")
